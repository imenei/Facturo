...
const STATUS_CONFIG: Record<string, { labelKey: string; color: string; icon: any }> = {
  ...
  en_cours:     { labelKey: 'status_in_progress', color: 'bg-blue-100 text-blue-700',      icon: Play },
...
function SignaturePad({ onSave, onCancel, t }: { onSave: (b64: string) => void; onCancel: () => void; t: (k: string) => string }) {
    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <h3 className="font-display font-700 text-slate-900">{t('client_signature')}</h3>
          <button onClick={onCancel} className="text-slate-400 hover:text-slate-600"><XCircle size={20} /></button>
        </div>
        <div className="p-5">
          <p className="text-sm text-slate-500 mb-3 text-center">{t('client_sign_confirmation')}</p>
          ...
          <div className="flex gap-3 mt-4">
            <button onClick={clear} className="btn-secondary flex-1 justify-center">{t('clear')}</button>
            <button onClick={save} disabled={!hasStrokes} className="btn-primary flex-1 justify-center">
              <PenLine size={15} /> {t('validate_signature')}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="card p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <h3 className="font-display font-600 text-slate-900">{t('worked_hours')}</h3>
      </div>
      ...
      <div className="text-xs text-center text-slate-400 mb-4">
        {t('already_saved')}: {savedMinutes} min · {t('this_session')}: {Math.ceil(elapsed / 60)} min
      </div>
...
function MaterialsEditor({ materials, onChange, t }: { materials: any[]; onChange: (m: any[]) => void; t: (k: string) => string }) {
...
export default function InterventionWorkspacePage() {
  ...
  const handleStart = async () => {
    ...
    try {
      ...
      setItem(data);
      setShowReport(true);
      toast.success(t('intervention_started'));
  ...
  const handlePause = async (sessionMinutes: number) => {
    ...
    try {
      ...
      setItem(data);
      toast(t('paused_report_saved'), { icon: '⏸️' });
    } catch { toast.error(t('error')); }
    setSaving(false);
  };
  ...
  const handleFinish = async (signature?: string) => {
    ...
    try {
      ...
      toast.success(t('intervention_completed'));
  ...
  const handleMarkNonRepaired = async () => {
    if (!confirm(t('mark_as_not_repaired_confirm'))) return;
    setSaving(true);
    try {
      await api.put(`/interventions/${id}`, { status: 'non_reparee' });
      toast(t('marked_not_repaired'), { icon: '❌' });
      router.push('/interventions');
    } catch { toast.error(t('error')); }
    setSaving(false);
  };
  ...
  const addPhoto = async (b64: string) => {
    ...
    catch { toast.error(t('error_saving_photo')); }
  };
  ...
  const saveSignature = async (b64: string) => {
    ...
    try {
      ...
      handleFinish(b64);
    } catch { toast.error(t('signature_error')); }
  };

  if (loading) return <div className="flex justify-center py-16"><Loader2 size={32} className="animate-spin text-brand-500" /></div>;
  if (!item) return <div className="p-8 text-slate-500">{t('intervention_not_found')}</div>;

  const { icon: StatusIcon, color: statusColor, labelKey: statusLabelKey } = STATUS_CONFIG[item.status] || STATUS_CONFIG.en_attente;
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto animate-fade-in">
      ...
      {showSignature && <SignaturePad onSave={saveSignature} onCancel={() => setShowSignature(false)} t={t} />}
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={clsx('badge flex items-center gap-1', statusColor)}>
                <StatusIcon size={11} />{t(statusLabelKey)}
              </span>
            </div>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="card p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-600 text-slate-500 uppercase tracking-wide">{t('client')}</span>
            </div>
          ...
          <div className="card p-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-600 text-slate-500 uppercase tracking-wide">{t('machine')}</span>
            </div>
          ...
          <div className="card p-4 bg-slate-50">
            <div className="flex items-center gap-2 text-slate-600">
              ...
              <span className="text-sm font-medium">
                {t('total_time')}: {Math.floor(item.workedMinutes / 60)}h {item.workedMinutes % 60}m
              </span>
              {item.startedAt && <span className="text-xs text-slate-400">· {t('started_on')} {new Date(item.startedAt).toLocaleDateString('fr-FR')}</span>}
              {item.finishedAt && <span className="text-xs text-slate-400">· {t('finished_on')} {new Date(item.finishedAt).toLocaleDateString('fr-FR')}</span>}
            </div>
          </div>
        )}
          ...
          <div className="card overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="font-display font-600 text-slate-900">{t('technical_report')}</span>
                {(diagnosis || actions) && (
                  <span className="badge bg-brand-100 text-brand-600 text-xs">{t('filled')}</span>
                )}
              </div>
            ...
            {showReport && (
              <div className="px-5 pb-5 space-y-4 border-t border-slate-100">
                ...
                <div className="pt-4">
                  <label className="label">{t('technical_diagnosis')}</label>
                  <textarea
                    ...
                    placeholder={t('technical_diagnosis_placeholder')}
                  />
                </div>

                <div>
                  <label className="label">{t('actions_performed')}</label>
                  <textarea
                    className="input resize-none"
                    rows={3}
                    value={actions}
                    ...
                    readOnly={!isEditable && isDone}
                    placeholder={t('actions_performed_placeholder')}
                  />
                </div>
                ...
                {/* Materials */}
                <div>
                  <label className="label">{t('materials_used')}</label>
                  {isEditable
                    ? <MaterialsEditor materials={materials} onChange={setMaterials} t={t} />
                    : materials.length > 0
                      ? (
                      ...
                      : <p className="text-sm text-slate-400 italic">{t('no_material')}</p>
                  }
                </div>
                ...
                {isEditable && (
                  <button onClick={handleSaveDraft} disabled={saving} className="btn-secondary w-full justify-center">
                    {saving ? <Loader2 size={14} className="animate-spin" /> : <><Save size={14} /> {t('save_report')}</>}
                  </button>
                )}
              </div>
            )}
        ...
        <div className="card p-5">
          <div className="flex items-center gap-2 mb-3">
            ...
            <h3 className="font-display font-600 text-slate-900">{t('photos')}</h3>
        ...
        {item.clientSignature && (
          <div className="card p-5">
            <div className="flex items-center gap-2 mb-3">
              ...
              <h3 className="font-display font-600 text-slate-900">{t('client_signature')}</h3>
              {item.signedAt && (
        ...
        {isTech && !isDone && (
          <div className="space-y-3">
            ...
            >
              {saving ? <Loader2 size={18} className="animate-spin" /> : <><PenLine size={18} /> {t('finish_with_client_signature')}</>}
            </button>
            ...
            >
              <CheckCircle size={16} /> {t('finish_without_signature')}
            </button>
            ...
            >
              <XCircle size={15} /> {t('not_repairable')}
            </button>
          </div>
        )}
          ...
          <div className={clsx('card p-5 border-2', item.status === 'terminee' ? 'border-emerald-200 bg-emerald-50' : 'border-red-200 bg-red-50')}>
            <div className="flex items-center gap-2">
              ...
              <span className={clsx('font-display font-700', item.status === 'terminee' ? 'text-emerald-700' : 'text-red-600')}>
                {item.status === 'terminee' ? t('intervention_done') : t('status_not_repaired')}
              </span>
            </div>
            {item.actualExitDate && (
              <p className="text-sm text-slate-500 mt-1">
                {t('closing_date')}: {new Date(item.actualExitDate).toLocaleDateString('fr-FR')}
              </p>
            )}
