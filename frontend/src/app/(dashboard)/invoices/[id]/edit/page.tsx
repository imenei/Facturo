...
export default function EditInvoicePage() {
  ...
  useEffect(() => {
    api.get(`/invoices/${id}`).then(({ data }) => {
      ...
      setLoading(false);
    }).catch(() => { toast.error('Erreur'); router.push('/invoices'); });
  }, [id]);
  ...
  return (
    <div className="p-6 md:p-8 max-w-4xl mx-auto animate-fade-in">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-3xl font-display font-700 text-slate-900">Modifier le document</h1>
      </div>
      <form onSubmit={handleSubmit} className="space-y-6">
        {/* Status */}
        <div className="card p-6">
          <h2 className="font-display font-600 text-slate-900 mb-4">Statut</h2>
        ...
        <div className="card p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="md:col-span-2"><label className="label">Nom *</label><input className="input" required value={form.clientName||''} onChange={(e) => setForm({ ...form, clientName: e.target.value })} /></div>
            ...
            <div><label className="label">Téléphone</label><input className="input" value={form.clientPhone||''} onChange={(e) => setForm({ ...form, clientPhone: e.target.value })} /></div>
            <div className="md:col-span-2"><label className="label">Adresse</label><input className="input" value={form.clientAddress||''} onChange={(e) => setForm({ ...form, clientAddress: e.target.value })} /></div>
            <div><label className="label">NIF</label><input className="input" value={form.clientNif||''} onChange={(e) => setForm({ ...form, clientNif: e.target.value })} /></div>
            <div><label className="label">NIS</label><input className="input" value={form.clientNis||''} onChange={(e) => setForm({ ...form, clientNis: e.target.value })} /></div>
          </div>
        </div>
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-display font-600 text-slate-900">Articles</h2>
            <button type="button" onClick={addItem} className="btn-secondary text-sm py-1.5"><Plus size={15} /> Ajouter</button>
          </div>
          <div className="space-y-2">
            {items.map((item, i) => (
              <div key={i} className="grid grid-cols-12 gap-2 items-center bg-slate-50 rounded-lg p-2">
                <div className="col-span-12 md:col-span-6"><input className="input bg-white" value={item.description} onChange={(e) => updateItem(i, 'description', e.target.value)} required /></div>
                <div className="col-span-4 md:col-span-2"><input className="input bg-white text-center" type="number" min={1} value={item.quantity} onChange={(e) => updateItem(i, 'quantity', Number(e.target.value))} /></div>
                <div className="col-span-7 md:col-span-3"><input className="input bg-white text-right" type="number" min={0} step="0.01" value={item.unitPrice} onChange={(e) => updateItem(i, 'unitPrice', Number(e.target.value))} /></div>
          ...
          <div className="mt-4 border-t pt-4 space-y-1.5 text-right">
            <div className="text-sm text-slate-600">HT: <span className="font-medium">{subtotal.toLocaleString('fr-DZ')} DZD</span></div>
            ...
            <div className="text-lg font-display font-700 text-brand-600">TOTAL: {total.toLocaleString('fr-DZ')} DZD</div>
          </div>
        </div>
        ...
        <div className="flex gap-3 justify-end">
          <Link href={`/invoices/${id}`} className="btn-secondary">Annuler</Link>
          <button type="submit" disabled={saving} className="btn-primary px-6">
            {saving ? <><Loader2 size={16} className="animate-spin" /> Sauvegarde…</> : <><Save size={16} /> Enregistrer</>}
          </button>
        </div>
      </form>
    </div>
